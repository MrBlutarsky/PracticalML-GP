...
My first idea was to produce feature plots, hoping to spot features well separating the different classes. However I gave it up quite soon: I tried some variables combinations but it always resulted in one class point covering up all the figure, with not much separation. 
...
The reason of choice 1 being to overcome the readability problem I had with the feature plots. The reason for point 2 is that I made the hypothesis that the same exercise class performed by persons with different physical attributes would result in different measurement ranges for a given feature. 
...
With which I get a (scary) 100% accuracy on the training set. Training such a model took a minute and a half on my laptop. The test set prediction accuracy is:
